...
--master yarn \
--name "[Squad FI] Motor de Ingestao Diario - SIAC promocao - BI03DD33.sh" \
--executor-memory 20G \
--executor-cores 4 \
--num-executors 9 \
...
--queue "SquadFI" \
--jars /projetos/plfinanceiro/bin/Libs/kudu-spark_2.10-1.5.0.jar /projetos/plfinanceiro/bin/MotorScala/dailySiacLoad/target/scala-2.10/dailysiacload_2.10-2.1.jar 6

impala-shell -k -i ${impala_shell_ip} -q "INVALIDATE METADATA siac_store_hist.ext_promocao;"