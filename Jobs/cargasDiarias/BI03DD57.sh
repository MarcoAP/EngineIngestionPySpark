...
spark-submit \
--master yarn \
--name "[Squad FI] - JOB BI03DD57 - Motor de ingestao Diario - RFZD_FINANCEIRO.SENHA_UTILIZADA - BI03DD57.sh" \
--executor-memory 20G \
--executor-cores 4 \
--num-executors 9 \
...
--queue "SquadFI" \
/projetos/plfinanceiro/bin/Datalake/Refined/RefinedMain.py -insert senha_utilizada $dt

impala-shell -k -i ${impala_shell_ip} -q "INVALIDATE METADATA rfzd_financeiro.senha_utilizada;"